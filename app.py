...
app.secret_key = os.environ.get(
    "FLASK_SECRET_KEY", "default_dev_secret_key_please_change"
)
...
try:
    ...
    print("Warning: config.json not found. Creating an empty user list.")
    users = []
...
def add_nonconformity():
    ...
    return render_template(
        "index.html", nonconformities=nonconformities, documents=documents
    )
...
def perform_audit():
    ...
    audit = Audit(
        audit_date=request.json["audit_date"],
...
def complete_corrective_action():
    ...
    corrective_action = CorrectiveAction(
        data["id"], data["description"], data["due_date"], data["assigned_to"]
    )
